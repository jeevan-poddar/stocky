-- Add dl_number to profiles if it doesn't exist
do $$
begin
  if not exists (select 1 from information_schema.columns where table_name = 'profiles' and column_name = 'dl_number') then
    alter table profiles add column dl_number text;
  end if;
end $$;

-- Add seller_dl_number to bills if it doesn't exist
do $$
begin
  if not exists (select 1 from information_schema.columns where table_name = 'bills' and column_name = 'seller_dl_number') then
    alter table bills add column seller_dl_number text;
  end if;
end $$;

-- Update the create_bill_transaction function to include seller_dl_number
create or replace function create_bill_transaction(
  ...
  p_seller_dl_number text default null
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  insert into bills (
    ...
    total_amount, total_profit, payment_mode, user_id, seller_dl_number
  ) values (
    p_invoice_number, p_customer_name, p_customer_phone, p_doctor_name, 
    p_total_amount, p_total_profit, p_payment_mode, auth.uid(), p_seller_dl_number
