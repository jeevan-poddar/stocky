-- 1. Add invoice_number column to bills table
ALTER TABLE bills 
ADD COLUMN IF NOT EXISTS invoice_number text UNIQUE;

-- 2. Update the create_bill_transaction function to accept and save invoice_number
create or replace function create_bill_transaction(
  ...
  p_payment_mode text,
  p_items jsonb,
  p_invoice_number text -- New parameter
)
returns uuid
language plpgsql
security definer
...
declare
  ...
  v_new_loose numeric;
begin
  -- 1. Insert Bill Header with Invoice Number
  insert into bills (
